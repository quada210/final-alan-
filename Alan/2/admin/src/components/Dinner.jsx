...
export const Dinner = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:4040/get3")
      .then(response => setUsers(response.data))
      .catch(err => console.log(err));
  }, []);

  const deleteValue = (id) => {
    axios.delete("http://localhost:4040/remove2/" + id)
      .then((response) => {
        alert(response.data);
  ...
  const handleUpdate = () => {
    axios.put(`http://localhost:4040/edit2/${updateId}`, updateData)
      .then((response) => {
        alert(response.data);
        window.location.reload(false);
      })
  ...
  return (
    <div style={{ margin: '4%' }}>
 <br /><br />     <Typography variant='h3'>Dinner items</Typography><br />
